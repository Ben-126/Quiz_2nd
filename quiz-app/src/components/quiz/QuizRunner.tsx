...
type EtatQuiz = "chargement" | "question" | "correction" | "termine" | "erreur";
...
function normaliserReponse(reponse: string | boolean): string {
  ...
  return reponse.toLowerCase().trim();
}

function verifierReponse(question: Question, reponseUser: string | boolean): boolean {
  ...
  const repUser = normaliserReponse(reponseUser);
  const repCorrecte = normaliserReponse(question.reponseCorrecte);
  return repUser.includes(repCorrecte) || repCorrecte.includes(repUser);
}
...
export default function QuizRunner({ matiereSlug, chapitreSlug, titreChapitre }: QuizRunnerProps) {
  ...
  const [derniereReponse, setDerniereReponse] = useState<{ reponse: string | boolean; correcte: boolean } | null>(null);
  ...
  const handleReponse = (reponse: string | boolean) => {
    if (etat !== "question") return;
    const question = questions[questionIndex];
    const correcte = verifierReponse(question, reponse);
